...
class WeightedPriorities:
    """
    WeightedPriorities class generates random priorities based on a weighted scheme.

    Attributes:
    - priorityChoiceWeights: Dictionary of priority weights for different schemes.
    - choiceType: Type of priority distribution ('even', 'high', 'low').
    - priorityChoiceList: List containing the weighted priorities.
    - nextPriority: Index to walk through priorityChoiceList.

    Methods:
    - __init__: Initializes the WeightedPriorities with default values and generates weighted priorities.
    - generateWeightedPriority: Generates a random priority list based on the specified scheme.
    - getNext: Gets the next priority from the list.
"""
    ...
    def getNext(self):
        ...
        self.nextPriority = (self.nextPriority +
                             1) % len(self.priorityChoiceList)
        return p
...
def generate_file(**kwargs):
    ...
    maxIOBT = int(kwargs.get("maxIOBT", random.randint(
        int(minIOBT), int(minIOBT) + 5)))
    minNumBursts = int(kwargs.get("minNumBursts", random.randint(5, 8)))
    maxNumBursts = int(kwargs.get(
        "maxNumBursts", random.randint(
            int(minNumBursts) + 3, int(minNumBursts) + 8)
    ))
    ...
    while process_id < nj:
        ...
        for job in range(jobs):
            ...
            cpub = random.randint(
                minNumBursts, maxNumBursts - 1)  # num cpu bursts
